...
# Ref: https://github.com/k3d-io/k3d-demo/blob/main/scripts/common.sh
...
# Check if K3D_CLUSTER_NAME is set
if [ -z "${K3D_CLUSTER_NAME}" ]; then
  echo -e "${RED}Error: K3D_CLUSTER_NAME environment variable is not set${END}"
  echo "Please set it using: export K3D_CLUSTER_NAME=your-cluster-name"
  exit 1
fi
